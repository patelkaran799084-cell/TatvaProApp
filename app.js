...
function bootAfterLogin() {
  ...
  // allowed email gate (non-admin)
  if (!isAdmin() && !isEmailAllowed(email)) {
    lockAppUI(true);
    showTopMsg("❌ Not allowed email");
    return;
  }

  // create business for user if missing
  ensureBusinessForUser(email);

  loadDBForActiveBusiness();
  renderHome();
}

// expose callback for gdrive.js
window.onDriveLoginSuccess = function () {
  hideTopMsg();
  bootAfterLogin();
};
...
(function init() {
  updateAdminUI();
  renderBizDropdown();
  lockAppUI(true); // locked until login
})(); 
